...
function startMenuScreen() {
  ...
  textSize(14);
  textStyle(NORMAL);
  text("Adapted by Ethan Davis", GAME_WIDTH / 2 - 80, GAME_HEIGHT / 2 + 120);
...
function difficultyMenuScreen() {
  ...
  textSize(14);
  textStyle(NORMAL);
  text("Easy challenege for most players", GAME_WIDTH / 2 - 140, GAME_HEIGHT / 2 - 75);
  ...
  textSize(14);
  textStyle(NORMAL);
  text("Balanced challenge for most players", GAME_WIDTH / 2 - 110, GAME_HEIGHT / 2 + 5);
  ...
  textSize(14);
  textStyle(NORMAL);
  text("Intense challenge, faster and tougher enemies", GAME_WIDTH / 2 - 140, GAME_HEIGHT / 2 + 85);
...
function spawnEnemy() {
  ...
  enemies.push({
    ...
    angle: 0,
    fireRate: random(80, 200) / settings.enemySpeed, // Faster firing rate on harder difficulties
    lastShot: frameCount
  });
}
      ...
      for (let j = enemies.length - 1; j >= 0; j--) {
        ...
        if (collideRectRect(bullet.x, bullet.y, bullet.width, bullet.height,
          ...
          if (enemy.health <= 0) {
            ...
            // Increased power-up drop chance on easy difficulty
